...
[![Tests](https://img.shields.io/badge/tests-802%20collected%2C%20789%20passing-brightgreen.svg)](#test-counts)
...
| **PyPI published releases** | Tag-push driven release workflow. `v1.1.0`, `v1.1.1`, `v1.1.2`, `v1.2.0`, `v1.2.1`, `v1.2.2`, `v1.3.0`, `v1.3.1`, and `v1.3.2` are live at https://pypi.org/project/nl2pbip/. Trusted publishing via OIDC — no API token stored in the repo. |
...
│   └── __init__.py
├── tests/                       # 802 pytest cases across 23 test files
...
Pytest collects **802 test cases across 23 test files** in CI (Python 3.10 / 3.11 / 3.12). Of those, **789 pass** on every supported Python version; the remaining 13 are skipped because the benchmarks in `tests/test_performance.py` are opt-in via `NL2PBIP_RUN_BENCHMARKS=1`. 3 additional tests in `tests/test_finetune.py` (not in the headline count) require the heavy `finetune` extra (`datasets` / `transformers` / `trl`) — install locally with `pip install ".[finetune]"` to run those 3. Run `pytest tests/ --no-header -q` to confirm locally.
